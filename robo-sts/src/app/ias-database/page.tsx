...
import { OfficerFormModal } from "@/app/ias-database/components/officer-form-modal";
import { OfficerViewModal } from "@/app/ias-database/components/officer-view-modal";
import { useIASDatabaseStore } from "@/store/ias-database-store";
...
const columnHelper = createColumnHelper<IASOfficer>();

// Complete list of all IAS Cadres / States in India
const ALL_CADRES = [
...
export default function IasDatabasePage() {
  const officers = useIASDatabaseStore((s) => s.officers);
  const isLoading = useIASDatabaseStore((s) => s.isLoading);
  const error = useIASDatabaseStore((s) => s.error);
  const totalCount = useIASDatabaseStore((s) => s.totalCount);
  const currentPage = useIASDatabaseStore((s) => s.currentPage);
  const pageSize = useIASDatabaseStore((s) => s.pageSize);
  const fetchOfficers = useIASDatabaseStore((s) => s.fetchOfficers);
  const updateOfficer = useIASDatabaseStore((s) => s.updateOfficer);
  const deleteOfficer = useIASDatabaseStore((s) => s.deleteOfficer);
  const bulkDelete = useIASDatabaseStore((s) => s.bulkDelete);
  const addOfficer = useIASDatabaseStore((s) => s.addOfficer);
  const selectedIds = useIASDatabaseStore((s) => s.selectedIds);
  const toggleSelected = useIASDatabaseStore((s) => s.toggleSelected);
  const clearSelection = useIASDatabaseStore((s) => s.clearSelection);
  ...
  const [viewOfficer, setViewOfficer] = useState<IASOfficer | null>(null);
  const [editingOfficer, setEditingOfficer] = useState<IASOfficer | null>(null);
  ...
  useEffect(() => {
    fetchOfficers(1, DEFAULT_PAGE_SIZE, {
      cadre: cadreFilter,
      state: stateFilter,
      status: statusFilter,
      search: debouncedSearch || undefined,
    });
  }, [debouncedSearch, cadreFilter, stateFilter, statusFilter, fetchOfficers]);
  ...
  const columns = useMemo(
    () => [
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-3 whitespace-nowrap">
            <img
              ...
              onError={(e) => {
                (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(row.original.name)}&background=6366f1&color=fff`;
              }}
            />
      ...
      columnHelper.accessor("officer_id", {
        header: "Officer ID",
        cell: ({ row }) => <span className="pl-2 font-mono text-xs">{row.original.officer_id}</span>,
      }),
      ...
      columnHelper.accessor("source", {
        header: "Source",
        cell: ({ row }) => <span className="pl-2">{row.original.source}</span>,
      }),
      columnHelper.accessor("domicile", {
        header: "Domicile",
        cell: ({ row }) => <span className="pl-2">{row.original.domicile}</span>,
      }),
      columnHelper.accessor("current_position", {
        header: "Present Post",
        cell: ({ row }) => (
          <span className="truncate block max-w-[250px]">{row.original.current_position}</span>
        ),
      }),
      columnHelper.accessor("pay_level", {
        header: "Pay Level",
        cell: ({ row }) => <span className="pl-2">{row.original.pay_level}</span>,
      }),
      columnHelper.accessor("central_deputation", {
        header: "Central Deputation",
        cell: ({ row }) => <span className="pl-2">{row.original.central_deputation}</span>,
      }),
      ...
      columnHelper.accessor("qualification", {
        ...
        cell: ({ row }) => (
          <span className="truncate block max-w-[200px]">{row.original.qualification}</span>
        ),
      }),
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" onClick={() => setViewOfficer(row.original)}>
              <Eye className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setEditingOfficer(row.original)}>
  ...
  const handleAdd = async (values: IASOfficerFormValues) => {
    setSubmitting(true);
    try {
      const officerData = {
        name: values.name || "",
        batch_year: values.batch_year || 2015,
        cadre: values.cadre || "",
        ...
        previous_postings: values.previous_postings
          ? values.previous_postings.split(/[,;]\s*/).filter(Boolean)
          : [],
        officer_id: values.officer_id || "",
        source: values.source || "",
        domicile: values.domicile || "",
        pay_level: values.pay_level || "",
        central_deputation: values.central_deputation || "",
  ...
  const handleEdit = async (values: IASOfficerFormValues) => {
    ...
    try {
      const officerData = {
        ...
        previous_postings: values.previous_postings
          ? values.previous_postings.split(/[,;]\s*/).filter(Boolean)
          : editingOfficer.previous_postings,
      };
      await updateOfficer(editingOfficer.id, officerData as Partial<IASOfficer>);
  ...
  const handleExport = async (type: "excel" | "pdf") => {
    try {
      ...
      const records = await pb.collection("ias_officers").getFullList({
        sort: "-created",
        filter,
      });
      ...
      const filename = `ias_officers_${new Date().toISOString().slice(0, 10)}`;
      if (type === "excel") {
        exportToCSV(`${filename}.csv`, headers, rows);
      } else {
        exportToPDF(`${filename}.pdf`, "IAS Officers Database", headers, rows, {
    ...
    <DashboardLayout>
      <div className="space-y-4">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">IAS Database</h1>
          ...
          <CardContent className="p-4">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Search by name, position, email, officer ID..."
                  value={searchInput}
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id}>
                      {headerGroup.headers.map((header) => (
                        <th key={header.id} className="px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-300 whitespace-nowrap">
